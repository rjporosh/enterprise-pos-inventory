...
| **Authentication / current user — STALE, corrected 2026-08-28** | `services/auth-service` now exists (added in commit `0e79624`, after this row was originally written) with real `/api/v1/auth/{register,login,refresh,logout,me,change-password,forgot-password,reset-password}`, OTP, security questions, audit logs, and RBAC admin endpoints (`/admin/{permissions,modules,roles}`). Neither frontend app calls it yet — `CashierId` in POS is still a raw pasted GUID. | Point each app at a new `NEXT_PUBLIC_AUTH_API_URL`, add an auth slice + refresh-token interceptor to `lib/api/client.ts` in both apps, derive `cashierId`/`userId` from the decoded token instead of a text field. | `auth-service` needs `dotnet` to build/run/verify — unavailable in the sandbox that made this correction; see `needed-credentials.md`. | **High** |
